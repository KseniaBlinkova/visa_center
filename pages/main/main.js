...
// 1. ДОБАВЛЯЕМ ИМПОРТЫ (Проверь, что файлы существуют по этим путям!)
...
export class MainPage {
    constructor(parent) {
        this.parent = parent;
        this.visaData = []; // Изначально пусто, данные придут с сервера
    }

    // 2. ЗАМЕНЯЕМ СТАРЫЙ getData НА СЕТЕВОЙ ЗАПРОС
    getData(searchTerm = "") {
        // Формируем URL с фильтрацией по заголовку (Вариант 1)
        ...
        ajax.get(url, (data, status) => {
            if (status === 200) {
                this.visaData = data; 
                this.renderCards(this.visaData); // Рисуем полученные данные
            } else {
    ...
    // 3. УДАЛЕНИЕ ТЕПЕРЬ ТОЖЕ ЧЕРЕЗ СЕРВЕР (Вариант 1)
    deleteVisa(id) {
        ajax.delete(visaUrls.getVisaById(id), (data, status) => {
            if (status === 200 || status === 204) {
                // После удаления просим сервер прислать обновленный список
                this.getData();
            }
        });
    }
    ...
    renderCards(data) {
        ...
        data.forEach(item => {
            ...
            card.render(
                item, 
                // В 5 лабе на страницу визы передаем только ID
    ...
    render() {
        ...
        // 4. ПОИСК ТЕПЕРЬ ВЫЗЫВАЕТ СЕТЕВОЙ ЗАПРОС
        new SearchBar(document.getElementById('search-placeholder'), (term) => {
            this.getData(term); 
        }).render();

        new ButtonAdd(document.getElementById('add-btn-placeholder')).render(() => {
            console.log("Добавление будет в 6 лабе");
        });

        // 5. ЗАПУСКАЕМ ПЕРВИЧНУЮ ЗАГРУЗКУ ДАННЫХ
        this.getData();
    }
}